  ...
  public func update(deltaTime : TimeInterval, foodLocation: CGPoint) {
    ...
    if timeSinceLastHit >= maxFlailTime {
      if action(forKey: walkingActionKey) == nil {
        ...
        if (abs(foodLocation.x - position.x) < 2 && abs(foodLocation.y - position.y) < 2) {
          physicsBody?.velocity.dx = 0
          removeAction(forKey: walkingActionKey)
          texture = walkFrames[1]
        } else if position.y - foodLocation.y > 2 {
            //Food is down
            physicsBody?.velocity.dy = -movementSpeed
            //yScale = -1
        } else if foodLocation.y - position.y > 2 {
            //Food is up
            physicsBody?.velocity.dy = movementSpeed
            //yScale = 1
        } else if position.x - foodLocation.x > 2 {
          //Food is left
